...
namespace Rash_Airlines.Controllers
{
    public class Passenger_LoginController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Login_Passenger(LoginUser login)
        {
            if (ModelState.IsValid)
            {
                var res = (from c in db.Passenger_Reg
                           where c.email == login.email && c.pwd == login.pwd
                           select c).SingleOrDefault();
                Session["Username"] = (from c in db.Passenger_Reg
                                       where c.email == login.email && c.pwd == login.pwd
                                       select c.first_name).SingleOrDefault(); ;
